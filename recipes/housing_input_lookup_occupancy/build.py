...
def download(table_name):
    beg_ts = time.time()
    url = 'https://raw.githubusercontent.com/NYCPlanning/db-developments/master/developments_build/data/housing_input_lookup_occupancy.csv'
...
def replace_header(table_name):
    file_path = Path(__file__).parent/'tmp'/f'{table_name}.csv'
    cmd = f"sed -i \"1s/.*/dobgeneraldesc,doboccupancycategorization,doboccupancycode2008,doboccupancycode1968,dobdetaileddescription,buildingcode,dcpclassificationold,dcpclassificationnew/\" {file_path}"
    os.system(cmd)
...
if __name__ == '__main__':
    table_name = 'housing_input_lookup_occupancy'
    download(table_name)
