...
def replace_header(table_name):
    file_path = Path(__file__).parent/'tmp'/f'{table_name}.csv'
    cmd = f"sed -i \"1s/.*/Jobnum,EffectiveDate,BIN,BoroName,HouseNumber,StreetName,Block,Lot,numofDwellingUnits,OccupancyClass,CertificateType,BuildingTypeDesc,DocStatus/\" {file_path}"
    os.system(cmd)
